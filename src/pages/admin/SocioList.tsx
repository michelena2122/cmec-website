...
const SocioList = () => {
  ...
  React.useEffect(() => {
    ...
    if (storedCuota) {
      try {
        const c = JSON.parse(storedCuota);
        setMontoCuota(c.monto || '');
        setLinkCuota(c.link || '');
      } catch {}
    }
  }, []);
  ...
  const guardarCuota = (monto: string, link: string) => {
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    Papa.parse(file, {
      complete: (results: any) => {
        const rows = results.data.slice(1).filter((row: any[]) => row.some(cell => cell));
    ...
    <div className="space-y-6">
      ...
      <div className="flex bg-white rounded-2xl p-1 border border-slate-200">
        {[
          ...
          { id: 'cartas', label: 'Configuración de Cartas' },
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl font-bold text-sm transition-all ${
              activeTab === tab.id ? 'bg-sky-900 text-white' : 'text-slate-500 hover:bg-slate-50'
            }`}
          >
            {tab.label}
          </button>
          ...
          <div className="p-6 border-b border-slate-50 bg-slate-50/50">
            <div className="relative max-w-sm w-full">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3 border border-slate-200 rounded-2xl bg-white focus:outline-none focus:ring-2 focus:ring-sky-500/20 text-sm"
                value={searchTerm}
                  ...
                  <tr key={socio.id} className="hover:bg-slate-50 transition-colors group">
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-3">
                        <div className="w-9 h-9 rounded-full bg-sky-100 flex items-center justify-center text-sky-700 font-bold text-xs uppercase">
                          {socio.nombre?.slice(0, 2)}
                        </div>
                        <span className="font-bold text-slate-800 text-sm">
                          {socio.prefijo} {socio.nombre}
                        </span>
                      </div>
                    </td>
                    ...
                    <td className="px-6 py-4">{socio.cuota2024 ? '✓' : '—'}</td>
                    <td className="px-6 py-4">{socio.cuota2025 ? '✓' : '—'}</td>
                    <td className="px-6 py-4">{socio.cuota2026 ? '✓' : '—'}</td>
                    <td className="px-6 py-4 text-right">
                      <button
                        onClick={() => borrar(socio.id)}
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <input
                ...
                onChange={e => {
                  ...
                  guardarCuota(e.target.value, linkCuota);
                }}
            ...
            <div>
              ...
              <input
                value={linkCuota}
                onChange={e => {
                  setLinkCuota(e.target.value);
                  guardarCuota(montoCuota, e.target.value);
                }}
      ...
      <div className="bg-white rounded-3xl border border-slate-200 p-8">
        ...
        <div className="grid grid-cols-2 gap-4 mb-4">
          <div>
            ...
            <input
              value={data.nombrePresidente || ''}
              onChange={e => guardar({ nombrePresidente: e.target.value })}
        ...
        <div className="space-y-4">
          <div>
            ...
            <input
              ...
              onChange={e => guardar({ domicilio: e.target.value })}
              placeholder="Av. Ejército Nacional..."
              className="w-full px-4 py-2.5 border border-slate-200 rounded-xl text-sm"
            />
          </div>
